...
namespace S3D {
    ...
    void DocumentImpl::create(const std::shared_ptr<Edge> edge) {
        ...
        this->graph()->connect(edge->from->id(), edge->to->id());
        this->regenMesh();
    }
    ...
    void DocumentImpl::remove(const std::shared_ptr<Edge> edge) {
        ...
        this->graph()->disconnect(edge->from->id(), edge->to->id());
        this->regenMesh();
    }
    ...
    void DocumentImpl::update(const std::shared_ptr<Node> node) {
        ...
        if (auto sphere = std::dynamic_pointer_cast<Sphere>(node)) {
            ...
            this->graph()->access(sphere->id(), [&sphere](auto sphr) {
                std::dynamic_pointer_cast<Sphere>(sphr)->coord = sphere->coord;
                std::dynamic_pointer_cast<Sphere>(sphr)->radius = sphere->radius;
            });
        } else if (auto setOp = std::dynamic_pointer_cast<SetOp>(node)) {
            this->interactor_->upsert(setOp->uid, Attribute{setOp->type}, now);
            this->graph()->access(sphere->id(), [&setOp](auto stp) {
                std::dynamic_pointer_cast<SetOp>(stp)->type = setOp->type;
            });
        }
        this->regenMesh();
    }
